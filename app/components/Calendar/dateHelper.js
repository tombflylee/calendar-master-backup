 ...
 * @param  {[type]} today [date对象]
 */
export const getDaysInMonth = (today) =>{
...
/**
 * 得到某一月所有天数按周组成若干个数组,一共5周
 * @param  {[type]} today [description]
 * @return {[type]}       [description]
 */
export const getFullWeekArray = (today) =>{
  let _currentDaysInMonth = getDaysInMonth(today);//该月份天数
  let _firstDay = getFirstDayOfMonth(today).getDay();//该月份1日的星期数
  let _weekArr = [];
  for(let i=0;i<5;i++){
    switch(i){
      case 0:{
